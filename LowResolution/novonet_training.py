...
def decode_spectrum(encoded_spectrum,
  ...
  with variable_scope.variable_scope(scope):
    ...
    if (data_utils.FLAGS.shared): # shared-weight
      dense1_input_size = data_utils.num_ion * data_utils.RESOLUTION
    ...
    #   
      ...
      # conv1: [128,8,10,26] >> [128,8,10,32] with kernel [1,3,26,32]
      conv1_weights = tf.get_variable(name="conv1_weights", 
                                      shape=[1,3,data_utils.vocab_size,32],
                                      initializer=tf.uniform_unit_scaling_initializer(1.43))
      conv1_biases = tf.get_variable(name="conv1_biases", 
                                     shape=[32],
                                     initializer=tf.constant_initializer(0.1))

      # conv2: [128,8,10,32] >> [128,8,10,32] with kernel [1,2,32,32]
      ...
      # max_pool: [128,8,10,32] >> [128,8,5,32]

      # dense1: # 4D >> [128,512]
      dense1_input_size = data_utils.num_ion * (data_utils.RESOLUTION // 2) * 32 # data_utils.vocab_size
    ...
    for i, AA_2 in enumerate(decoder_inputs_emb):
      ...
      #   
        ...
        # image_batch: [128,26,8,10] >> [128,8,10,26]
        # This is a bug, should be fixed at the input processing later.
        image_batch = tf.transpose(candidate_intensity, perm=[0,2,3,1])  # [128,8,10,26]
        ...
        conv2 = tf.nn.max_pool(conv2, ksize=[1,1,3,1],strides=[1,1,2,1], padding='SAME') 
...
def encode_spectrum(encoder_inputs, 
  ...
  with variable_scope.variable_scope("embedding_rnn_seq2seq"):
    ...
    # CNN1
    cnn1_size = data_utils.encoding_cnn_size
    cnn1_filter = data_utils.encoding_cnn_filter
    cnn1_W = variable_scope.get_variable(name="cnn1_W", shape=[1,cnn1_size,1,cnn1_filter],
                                        initializer=tf.uniform_unit_scaling_initializer(1.43))
    cnn1_B = variable_scope.get_variable(name="cnn1_B", shape=[cnn1_filter],
                                        initializer=tf.constant_initializer(0.1))
    layer_cnn1 = tf.nn.relu(tf.nn.conv2d(layer0, cnn1_W, strides=[1,1,1,1], padding='SAME') + cnn1_B)
    layer_cnn1_pool = tf.nn.max_pool(layer_cnn1, ksize=[1,1,cnn1_size,1], strides=[1,1,cnn1_size,1], padding='SAME')
    layer_cnn1_drop = tf.nn.dropout(layer_cnn1_pool, keep_conv)

    # DENSE1
    dense1_input_size = 1 * (data_utils.MZ_SIZE // cnn1_size) * cnn1_filter
    dense1_output_size = data_utils.num_units
    #
    ...
    dense1_B = variable_scope.get_variable(name="dense1_B", shape=[dense1_output_size],
                                           initializer=tf.constant_initializer(0.1))
    #
    layer_dense1_flat = tf.reshape(layer_cnn1_drop, [-1, dense1_input_size])
    layer_dense1 = tf.nn.relu(tf.matmul(layer_dense1_flat, dense1_W) + dense1_B)
    layer_dense1_drop = tf.nn.dropout(layer_dense1, keep_dense)

    # SPECTRUM as Input 0
    #
    encoded_spectrum = layer_dense1_drop
    #~ #
