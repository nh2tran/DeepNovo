...
#~           if (peptide[-1]=="C" and raw_sequence[index:index+8]=="(+58.01)"): # orbi
      ...
      while not ("END IONS" in line):
        ...
        if (mz_float > data_utils.MZ_MAX): # skip this peak
          ...
          continue
        #
        spectrum_mz.append(mz_float) # shift
...
def knapsack_search(knapsack_matrix, peptide_mass, mass_precision_tolerance):
  ...
  #~ return range(3, 26) # test knapsack
...
def decode_beam_search_2(sess, model, data_set, knapsack_matrix):
  ...
  candidate_mass_list.append([prefix_mass_list, 
                              suffix_mass_list, 
                              data_utils.PRECURSOR_MASS_PRECISION_TOLERANCE, # precursor_mass_precision
                              data_utils.KNAPSACK_MASS_PRECISION_TOLERANCE # knapsack_precision
                              ])
  ...
  for spectrum_id in xrange(data_set_len):
    ...
    peptide_mass_C_location = int(round(peptide_mass_C*data_utils.RESOLUTION))
    ...
    #
    argmax_mass = argmax_location / data_utils.RESOLUTION # !!! LOWER precision 0.1 Da !!!
    #
